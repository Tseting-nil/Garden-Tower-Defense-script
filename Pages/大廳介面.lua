...
-- === Tab_event（活動）：自動採花 / 自動撿取 === --
-- 內嵌兩支活動腳本原始碼（與 自動採花.lua / 自動撿取.lua 同步；改那兩檔記得同步這裡）。
do
	local FLOWER_SRC = [==[
--[[
	Garden Tower Defense — 自動採花 (Flower Harvest)
	================================================
	逐朵走到地圖上的花 (tag = "Collectables_flower")，FireServer 採收。
	採完全場的花 → server 在你身上生一個幸運方塊 (lucky26_flower)，
	再交給「自動撿取.lua」收掉 (或本檔結束時提示)。

	呼叫鏈 (反編譯來源 SharedCollectables.MapModelBehaviours.flower)：
	  client FlowerPrompt.Triggered (Line 189)
	    -> RemoteEvents.CollectablesFlowerCollect:FireServer(flowerPart)
	  server (Line 94)：驗 1)合法花 2)玩家未在冷卻 3)距離 <= PromptDistance(預設12)+8 ≈ 20
	                    4)同朵花每人限一次；全採完 -> SpawnCollectable 幸運方塊
	  server 回 CollectablesFlowerCollected(花, 剩餘數) -> client 拔花動畫 + "N left!"

	用走路而非瞬移 (GTD 伺服器有 ExploitDetector 反作弊)，移動用巡路模組 Move API。

	停止：getgenv().__AUTO_FLOWER.stop()
]]

if getgenv().__AUTO_FLOWER then
	getgenv().__AUTO_FLOWER.stop()
end

-- ===== 設定 =====
local MOVE_URL = "https://gist.githubusercontent.com/Tseting-nil/494a4830fa6d3466596e4e01ca25bdee/raw/15a370f3f09720e1359601e05da6d9e0a24bcc23/%E5%B7%A1%E8%B7%AF%E6%A8%A1%E7%B5%84"
local TAG = "Collectables_flower"
local FIRE_DISTANCE = 14      -- 走到離花這麼近就 FireServer (server 上限約 20，留安全邊際)
local PER_FLOWER_TIMEOUT = 15 -- 走向單朵花的逾時 (秒)，逾時就跳過
local RETURN_HOME = true      -- 採完是否走回出發點
...
local RE_Collect = RE:WaitForChild("CollectablesFlowerCollect")
local RE_Collected = RE:WaitForChild("CollectablesFlowerCollected")
...
local okMove, Move = pcall(function()
	return loadstring(game:HttpGet(MOVE_URL))()
end)
if not okMove or type(Move) ~= "table" then
	return warn("[AutoFlower] 巡路模組載入失敗：" .. tostring(Move))
end
Move:SetPathVisualization(false) -- 不留路徑方塊
...
-- 監聽 server 回報剩餘數 (順便印 log)
conns[#conns + 1] = RE_Collected.OnClientEvent:Connect(function(flowerPart, remaining)
	picked = picked + 1
	print(("[AutoFlower] 🌸 採到一朵，剩餘 %s 朵 (本次已採 %d)"):format(tostring(remaining), picked))
	if tonumber(remaining) == 0 then
		print("[AutoFlower] ✅ 全部採完！幸運方塊已生成，交給 自動撿取.lua 收取。")
	end
end)

-- 走到 part 附近 (走路，非瞬移)；回傳 "near" / "gone" / "timeout" / "stopped"
local function approach(part)
	local target = part.Position
	Move:NavigateToPosition(target.X, target.Y, target.Z)
	local t0 = os.clock()
	while running do
		if not part.Parent then return "gone" end
		local root = getRoot()
		if (root.Position - part.Position).Magnitude <= FIRE_DISTANCE then
			return "near"
		end
		if os.clock() - t0 > PER_FLOWER_TIMEOUT then return "timeout" end
		task.wait(0.1)
	end
	return "stopped"
end
...
local mainThread = task.spawn(function()
	local origin = getRoot().Position

	local flowers = {}
	for _, p in ipairs(CollectionService:GetTagged(TAG)) do
		if p:IsA("BasePart") then table.insert(flowers, p) end
	end
	print(("[AutoFlower] 開始，共偵測到 %d 朵花。停止：getgenv().__AUTO_FLOWER.stop()"):format(#flowers))

	-- 依距離由近到遠排序，少走冤枉路
	local fromPos = origin
	local remaining = table.clone(flowers)
	while running and #remaining > 0 do
		-- 找最近的一朵
		local bestI, bestD = 1, math.huge
		for i, f in ipairs(remaining) do
			if f.Parent then
				local d = (fromPos - f.Position).Magnitude
				if d < bestD then bestD, bestI = d, i end
			end
		end
		local flower = table.remove(remaining, bestI)
		if flower and flower.Parent then
			local res = approach(flower)
			if res == "near" then
				pcall(function() RE_Collect:FireServer(flower) end)
				fromPos = flower.Position
				task.wait(0.25) -- 給 server 處理 + 演出
			elseif res == "stopped" then
				break
			else
				print(("[AutoFlower] 跳過一朵 (%s)"):format(res))
				fromPos = flower.Position
			end
		end
	end

	Move:StopSingleNavigation()

	if running and RETURN_HOME then
		print("[AutoFlower] 採收結束，走回出發點…")
		Move:NavigateToPosition(origin.X, origin.Y, origin.Z)
		local t0 = os.clock()
		while running and os.clock() - t0 < 20 do
			if (getRoot().Position - origin).Magnitude <= 6 then break end
			task.wait(0.2)
		end
		Move:StopSingleNavigation()
	end

	print(("[AutoFlower] 完成，本次共採 %d 朵。"):format(picked))
	if getgenv().__AUTO_FLOWER then getgenv().__AUTO_FLOWER._done = true end
end)

getgenv().__AUTO_FLOWER = {
	stop = function()
		running = false
		for _, c in ipairs(conns) do pcall(function() c:Disconnect() end) end
		pcall(function() Move:StopSingleNavigation() end)
		pcall(function() task.cancel(mainThread) end)
		getgenv().__AUTO_FLOWER = nil
		warn("[AutoFlower] 已停止")
	end,
}
...
print(("[AutoCollect] 已啟動，場上現有 %d 個撿取物已嘗試收取；持續監聽 + 每 %ds 補掃。停止：getgenv().__AUTO_COLLECT.stop()")
	...
	Tab_event:Separator({ Text = L.event_flower_section })
	Tab_event:Checkbox({
		Label = L.event_flower_toggle,
		Value = false,
		Callback = function(self, on)
			if on then
				if runSrc(FLOWER_SRC, L.event_flower_section) then
					Msg:Success(L.event_flower_started)
					task.spawn(function()
						while getgenv().__AUTO_FLOWER do task.wait(1) end
						pcall(function() self:SetValue(false) end)
					end)
				else
					self:SetValue(false)
				end
				...
				if getgenv().__AUTO_FLOWER then
					pcall(function() getgenv().__AUTO_FLOWER.stop() end)
					Msg:Success(L.event_flower_stopped)
				end
			end
		end,
	})
